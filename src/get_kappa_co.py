...
from gc_utils import (  # noqa: F401 # type: ignore
    block_print,
    enable_print,
    get_halo_tree,
    main_prog_halt,
...
def kappa_co(part, snapshot: int, r_50: float, kappa_dict: dict = {}, log_t_max: float = 4.5):
    ...
    kappa_dict[snap_id] = {}
    kappa_dict[snap_id]["r_50"] = r_50
    kappa_dict[snap_id]["kappa_co"] = kappa_co
...
if __name__ == "__main__":
    ...
    kappa_dict = {}
    for idx in idx_main_lst:
        kappa_dict = call_func(idx, halt, fire_dir, kappa_dict)

    save_path = data_dir + "external/kappa_co.json"
    with open(save_path, "w") as json_file:
        json.dump(kappa_dict, json_file, indent=4)
